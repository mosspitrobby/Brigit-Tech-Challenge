...
export function formatInvalidNumber(key: string, min?: number, max?: number) {
  if (min && max) {
    return `\`${key}\` is missing or invalid (must be between ${Intl.NumberFormat(
      'en-AU',
    ).format(min)} - ${Intl.NumberFormat('en-AU').format(max)})`;
  } else if (max) {
    return `\`${key}\` is missing or invalid (must be less than ${Intl.NumberFormat(
      'en-AU',
    ).format(max)})`;
  } else if (min) {
    return `\`${key}\` is missing or invalid (must be greater than ${Intl.NumberFormat(
      'en-AU',
    ).format(min)})`;
  }
  return `\`${key}\` is missing or invalid`;
...
export const SUCCESS_DESCRIPTION = `Returns \`${SUCCESS}\` message and data when a request is completed successfully`;
